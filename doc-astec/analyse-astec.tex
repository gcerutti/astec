...
\item appel \`a \texttt{get\_seeds()} (section \ref{sec:get:seeds}, page \pageref{sec:get:seeds}). Pour chaque cellule, et pour un ensemble de valeurs de $h \in [h_{min}, h_{max}]$ (avec un pas de 2), \texttt{get\_seeds()} calcule le nombre de  $h$-minima qui sont strictement inclus dans la cellule $c$ de $\tilde{S}_{t+1}$. Cette op\'eration de s\'election des $h$-minima strictement inclus dans la cellule  $c$ de $\tilde{S}_{t+1}$ est faite plusieurs fois, on peut donc la mutualiser.
...
\item appel \`a \texttt{volume\_checking()}
(section \ref{sec:volume:checking}, page \pageref{sec:volume:checking})

\item appel \`a \texttt{outer\_correction()}
...
\end{verbatim}
\color{\mycolor}
Plut\^ot que de faire une boucle sur les cellules, on pourrait juste masquer \verb|seg_from_opt_h| par le masque de l'ouverture.
\color{black}
\begin{verbatim}
