...
\item The line
\begin{verbatim}
fusion\_method = 'direct-fusion'
\end{verbatim}
will set the co-registration strategy to the one described in \cite{guignard:tel-01278725,guignard:hal-01938126}: each acquisition image is linearly co-registered with the referenceone, i.e. the one from the left camera and for the first stack.
\item The line
\begin{verbatim}
fusion\_method = 'hierarchical-fusion'
\end{verbatim}
...
\paragraph{Stacks non-linear co-registration}
...
\item \texttt{fusion\_method}  see section \ref{sec:cli:fuse:image:coregistration}
