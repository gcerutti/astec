...
\begin{itemize}
\item \texttt{'increasing'}: \textbf{z} increases from the high-contrasted images to the fuzzy ones (see figure \ref{fig:cli:fuse:spim:right:acquisition}).
\item \texttt{'decreasing'}: \textbf{z} increases from the fuzzy images to the high-contrasted ones (see figure \ref{fig:cli:fuse:spim:left:acquisition}).
\end{itemize}
...
\item \texttt{'ramp'}:
the weights are linearly increasing or decreasing along the \textbf{Z} axis (see figure \ref{fig:cli:fuse:ramp:combination}), depending on the value of the \texttt{acquisition\_leftcamera\_stacking\_direction}. If the latter is \texttt{'increasing'} (see section \ref{sec:cli:fuse:important:parameters}), the weights are decreasing along \textbf{Z}, and vice versa.
\item \texttt{'corner'}: the weights are linearly increasing or decreasing along the \textbf{Z} axis (see figure \ref{fig:cli:fuse:corner:combination}), depending on the value of the \texttt{acquisition\_leftcamera\_stacking\_direction}, while narrowing at the center of the image. It somehow mimics a stitching of the 4 resampled co-registered image stacks, where the information is kept from the most informative image.
\item \texttt{'guignard'}: 
